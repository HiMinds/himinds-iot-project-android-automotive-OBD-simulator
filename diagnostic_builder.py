...
class DiagnosticEventBuilder(object):
    class ByteArray(object):
        ...
        def _getIndices(self, bit):
            if (bit < 0) or (bit >= self.count):
                raise IndexError(
                    "index %d not in range [0,%d)" % (bit, self.count))
            byteIdx = bit / 8
            bitIdx = (bit % 8)
    ...
    def __init__(self, propConfig):
        ...
        print(self.numIntSensors)
        print(self.numFloatSensors)
        self.bitmask = DiagnosticEventBuilder.ByteArray(self.numIntSensors +
                                                        self.numFloatSensors)
